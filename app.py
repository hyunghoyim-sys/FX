...
with tab1:
    ...
    # [예측 로직 개선: 12월 하방 경직성 및 일일 변동성 제한]
    ...
    for i in range(1, future_days + 1):
        ...
        # 1. 일일 변동성 제한 (Daily Volatility Cap: 5원)
        max_move = 5.0 
        ...
        # 2. 2025년 12월 내 하락 방지 로직 (연말 수급 효과 반영)
        # 현재가 12월이고 예측 날짜도 12월이면 하락을 제한함 (하방 경직)
        if target_date.year == 2025 and target_date.month == 12:
            if move < 0:
                # 하락 압력이 있더라도 연말 수요로 인해 보합 또는 미세 상승으로 방어
                move = 0 
    ...
    st.info("💡 **Analyst Note:** AI 모델은 연말(12월) 결제 수요 및 북클로징 영향으로 인한 **하방 경직성**을 반영하고 있으며, 일일 변동폭을 최대 **5원** 내외로 제한하여 안정적인 예측 경로를 산출했습니다.")
