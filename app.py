...
# -----------------------------------------------------------------------------
# 4. 모델링 로직 (수식 강화: 기준금리 반영 확대)
# -----------------------------------------------------------------------------
# [New Normal 반영]
base_constant = 1380 

# 기준금리 차이(Spread) 계산 및 반영
rate_spread = user_us_rate - user_kr_rate 

# Fair Value(적정가) 계산
# 기준금리 스프레드 가중치를 대폭 상향 (25 -> 50)하여 슬라이더 조절 시 영향력 가시화
fair_value = (
    base_constant 
    + (rate_spread * 50)           # [Core] 한-미 금리차 (1%p 확대 시 50원 상승)
    + (user_us10y - 4.0) * 30      # 시장금리 보조 지표
    + (user_dxy - 100) * 20        # 달러 인덱스
    + (user_seohak - 50) * 1.2     # 서학개미 수급
    + (user_jpy - 150) * 1.5       # 엔화 동조화
    + (user_cny - 7.2) * 20.0      # 위안화 연동
    + 60 # Risk Premium
)
...
k2.metric("🏦 한-미 금리차", f"{rate_spread:.2f}%p", "핵심 상승 요인")
...
with tab1:
    ...
    for i in range(1, future_days + 1):
        ...
        # [Intervention] 1500원 저항
        if next_val > 1500:
             excess = next_val - 1500
             next_val = 1500 + (excess * 0.2)
...
# -----------------------------------------------------------------------------
# 6. 인포그래픽 (Enhanced with Rate Spread)
# -----------------------------------------------------------------------------
st.markdown("---")
st.markdown("### 📑 FX-AI Insight Report & Methodology")

# HTML 인포그래픽 (상관계수 매트릭스에 금리차 추가)
infographic_html = """
<!DOCTYPE html>
<html lang="ko">
<head>
...
<body>
    <div class="max-w-6xl mx-auto">
        ...
        <!-- 1. Correlation Matrix Analysis (Updated) -->
