...
# -----------------------------------------------------------------------------
# 1470원대 시장 상황을 반영하기 위해 DXY와 금리 민감도 계수를 현실화하고 Base를 조정
# Base Rate: 1300 -> 1320 (기초 체력)
# DXY Impact: 20 -> 25 (강달러 민감도 상향)
# US Rate Impact: 50 (유지)
# Bias Correction: +60 (최근 프리미엄 반영)
base_constant = 1320 
fair_value = (
    base_constant 
    + (user_us10y - 4.0) * 50      # 미국 금리
    + (user_dxy - 100) * 25        # 달러 인덱스 (103->100 기준점 변경하여 현실화)
    + (user_seohak - 50) * 1.2     # 서학개미 (영향력 상향)
    + (user_jpy - 900) * 0.5       # 엔화
    + (user_cny - 190) * 1.0       # 위안화
    + 60 # Market Risk Premium (지정학적 리스크 등 알파)
)
...
with tab1:
    ...
    # [차트 연결성 수정] 예측선이 실제 데이터의 마지막 점(Today)에서 시작하도록 수정
    # 날짜: [오늘, 내일, 모레, ...]
    # 가격: [현재가, 예측1, 예측2, ...]
    dates_future = [pd.Timestamp(last_date)] + [pd.Timestamp(last_date) + datetime.timedelta(days=x) for x in range(1, future_days+1)]
    
    # 가격 예측 시퀀스 생성 (현재가 -> 적정가로 부드러운 수렴)
    prices_future = [current_price] # 시작점은 무조건 현재가
    for i in range(1, future_days + 1):
        weight = i / future_days 
        # 로그 함수적 접근이 아닌 선형+모멘텀 혼합으로 자연스러운 곡선 유도
