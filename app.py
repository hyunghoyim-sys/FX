...
# -----------------------------------------------------------------------------
# 4. 모델링 로직 (Revised for Market Reality)
# -----------------------------------------------------------------------------
# 최근 환율의 구조적 상승(New Normal)을 반영하여 Base Constant와 계수를 현실화함
# 기존 Base 1300 -> 1350 상향, 금리/달러 민감도 대폭 강화
base_constant = 1350 
fair_value = (
    base_constant 
    + (user_us10y - 4.0) * 50      # 미국 금리 민감도 강화 (30 -> 50)
    + (user_dxy - 103) * 20        # 달러 인덱스 영향력 확대 (12 -> 20)
    + (user_seohak - 50) * 1.0     # 서학개미 수급 요인 반영 (0.8 -> 1.0)
    + (user_jpy - 900) * 0.5       # 엔화 동조화
...
k4.metric("🐜 서학개미 영향", f"{(user_seohak-50)*1.0:+.1f} 원", "환율 지지분")
...
with tab1:
    ...
    # [예측 로직 개선] 현실적인 추세 반영 (Momentum)
    # 갑작스러운 급락을 방지하기 위해 현재가 비중을 높이고 적정가로 천천히 수렴하도록 조정
    # Momentum Factor: 최근 트렌드를 3일 정도 유지하려는 관성 적용
    prices_future = []
    temp_price = current_price
    for i in range(1, future_days + 1):
        # 14일 동안 Fair Value로 수렴하되, 초기에는 천천히 이동 (Logarithmic convergence)
        weight = i / future_days 
        next_price = current_price * (1 - weight) + fair_value * weight
    ...
    st.info("💡 **Analyst Note:** 최근 1,400원 대 고환율 고착화(New Normal) 경향과 강력한 서학개미 매수세를 반영하여 모델의 하단 지지선을 상향 조정했습니다.")
