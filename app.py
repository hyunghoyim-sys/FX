...
# Fair Value(적정가) 계산 - USD 기준 통화 로직 반영
# 달러/엔 상승(엔화 약세) -> 원화 동반 약세 압력 -> 환율 상승 (+)
# 달러/위안 상승(위안화 약세) -> 원화 동반 약세 압력 -> 환율 상승 (+)
fair_value = (
    base_constant 
    + (user_us10y - 4.0) * 50      # 미국 금리
    + (user_dxy - 100) * 25        # 달러 인덱스
    + (user_seohak - 50) * 1.2     # 서학개미
    + (user_jpy - 150) * 1.5       # 엔화 (150엔 기준 1엔당 1.5원 영향)
    + (user_cny - 7.2) * 20.0      # 위안화 (0.1위안당 2원 영향)
...
k2.metric("🇯🇵 달러/엔", f"{user_jpy:.2f}", "USD/JPY")
k3.metric("🇨🇳 달러/위안", f"{user_cny:.2f}", "USD/CNY")
...
with tab1:
    ...
    # [예측 로직 개선: 당국 개입 경계감 및 박스권(1460~1480) 설정]
    prices_future = [current_price]
    current_val = current_price
    
    # 외환 당국 개입 경계 레벨 (Resistance)
    resistance_level = 1480
    # 하단 지지선 (Support)
    support_level = 1460
    ...
    for i in range(1, future_days + 1):
        ...
        # 기본 변동성 (Volatility)
        move = gap * 0.2 # 방향성 일부 반영
        noise = np.random.normal(0, 3.0) # 랜덤 등락
        
        next_val = current_val + move + noise
        
        # [Intervention Logic] 1475원 이상에서는 상승 압력 둔화 (스무딩)
        if next_val > 1475:
            # 상승분의 70%를 삭감 (개입 경계감)
            excess = next_val - 1475
            next_val = 1475 + (excess * 0.3)
            
        # [Box Range Clamp] 1460 ~ 1480 구간 제한
        if next_val > resistance_level:
            next_val = resistance_level
        elif next_val < support_level:
            next_val = support_level
    ...
    st.info("💡 **Analyst Note:** AI 모델은 외환 당국의 스무딩 오퍼레이션(미세 조정) 경계감과 1,480원 부근의 저항을 반영하여, 향후 3일간 1,460원~1,480원 구간 내에서의 제한적인 등락을 예측합니다.")
